...
int main(int argc, char** argv) {
    ...
    KinematicsEngine kinematics;
    while (run_loop && !network->start_server(Config::TCP_PORT)) {
        std::cerr << "CRITICAL: Port locked. Retrying...\n";
        usleep(2000000);
    }
    if (!run_loop) return 0;
    std::cout << "[MIXR-1] Standalone active. Dashboard listener ready on port " << Config::TCP_PORT << ".\n";
    ...
    while (run_loop) {
        ...
        bool had_client = network->is_client_connected();
        network->poll_for_client();
        bool has_client = network->is_client_connected();
        if (has_client && !had_client) {
            std::cout << "[MIXR-1] Dashboard Connected.\n";
            dashboard_notified = true;
        } else if (!has_client && dashboard_notified) {
            std::cout << "[MIXR-1] Dashboard Disconnected. Continuing standalone control.\n";
        ...
        if (simulink_is_active) {
            ...
            if (network->is_client_connected()) network->send_packet(-2.0, -2.0, -2);
            continue;
        }
        ...
        if (network->is_client_connected() && network->receive_command(target_rpm, target_pwm_pct, pi_mode)) {
        ...
        if (update_net && network->is_client_connected()) {
            network->send_packet(state.exact_rpm, state.ema_filtered_rpm, encoder->get_revolutions());
        }
