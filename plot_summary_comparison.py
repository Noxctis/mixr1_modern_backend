...
def plot_metric(datasets, key, ylabel, title, output_path, log=False):
...
def write_report(path, datasets):
...
def main():
    ...
    axis.set_title("Motor speed and RPM variation: FIFO versus No-FIFO")
    axis.set_xlabel("PWM (%)")
    ...
    plot_metric(datasets, "std_raw_rpm", "RPM standard deviation", "RPM standard deviation", os.path.join(args.output, "rpm_standard_deviation.png"))
    figure, axis = plt.subplots(figsize=(10, 6))
    for rows in datasets:
        pwm = [row["pwm_percent"] for row in rows]
    ...
    plt.close(figure)
    plot_metric(datasets, "mean_period_us", "Mean period (us)", "Mean control-loop period", os.path.join(args.output, "mean_control_loop_period.png"))
    figure, axis = plt.subplots(figsize=(10, 6))
    for rows in datasets:
        pwm = [row["pwm_percent"] for row in rows]
