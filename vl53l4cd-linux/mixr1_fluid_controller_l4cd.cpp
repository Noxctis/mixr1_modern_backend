// PORT NOTE (VL53L0X -> Pololu/ST VL53L4CD on the Pi, via VL53L4CD.hpp):
//   * Only the sensor class name/header changed; every sensor call
//     (initialize, setTimeout, setMeasurementTimingBudget, readRangeSingleMillimeters,
//     timeoutOccurred) keeps the same name, and bad reads still come back as 65535.
//   * LEVEL_GAIN_CORRECTION was reset from 0.93738 to 1.0. That number was fitted to
//     VL53L0X data (revision note #3) and is NOT valid for a different sensor. Re-run
//     the tank zero experiment with the VL53L4CD and re-fit before enabling it again.
//     Note the failure direction: keeping 0.93738 on a sensor that does not have the
//     same +6.6% bias makes calculated level read LOW, so the pump would OVERSHOOT
//     the target. 1.0 errs toward stopping early instead.
//   * container_zero.txt holds VL53L0X-era numbers -- delete it and re-run
//     "Set Container & Floater (Calibration)" after remounting the new sensor.
//   * Datasheet note (DS13812): 18 deg FoV / 1200 mm max range / 1 mm min range.
//
...
constexpr double LEVEL_GAIN_CORRECTION = 1.0; // was 0.93738 (VL53L0X fit) -- re-fit for VL53L4CD
...
void capture100Readings(VL53L4CD& sensor, const std::string& eventName, double containerZero, double floaterThickness) {
...
SensorMetrics getSensorMetrics(VL53L4CD& sensor, int samples, int delay_us = 10000) {
...
void runCalibration(VL53L4CD& sensor, double& containerZero, double& floaterThickness) {
    std::string dummy;
...
void runContinuousRead(VL53L4CD& sensor, double containerZero, double floaterThickness) {
    if (containerZero == 0.0) {
...
void runTankZeroExperiment(VL53L4CD& sensor) {
...
void runSolenoidAndToF(VL53L4CD& sensor, double containerZero, double floaterThickness) {
    if (containerZero == 0.0) {
        std::cout << "[!] Run calibration first.\n";
        return;
    }
...
void runFullFluidCycle(VL53L4CD& sensor, double containerZero, double floaterThickness) {
    if (containerZero == 0.0) {
        std::cout << "[!] Run calibration first.\n";
        return;
    }
...
void menuHardware(VL53L4CD& sensor, double& containerZero, double& floaterThickness) {
    int choice = 0;
    while (!systemOffline) {
...
void menuFillTests(VL53L4CD& sensor) {
    int choice = 0;
    while (!systemOffline) {
        std::cout << "\n--- FILL TESTS ---\n";
...
void menuDrainTests(VL53L4CD& sensor, double containerZero, double floaterThickness) {
    int choice = 0;
    while (!systemOffline) {
        std::cout << "\n--- DRAIN TESTS ---\n";
...
void menuFillAndDrainTests(VL53L4CD& sensor, double containerZero, double floaterThickness) {
    int choice = 0;
    while (!systemOffline) {
        std::cout << "\n--- FILL AND DRAIN TESTS ---\n";
...
int main() {
    ...
    try {
        ...
        sensor.setMeasurementTimingBudget(200000); // VL53L4CD range is 10-200 ms; 50000 = 4x faster loop
    } catch (...) {
